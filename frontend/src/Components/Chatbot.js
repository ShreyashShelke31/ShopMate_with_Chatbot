...
// export default Chatbot;
const allowedCommands = ["help", "products", "cart", "contact"];

const sendMessage = async () => {
  if (!input) return;

  if (!allowedCommands.includes(input.toLowerCase())) {
    setMessages((prev) => [
      ...prev,
      { role: "bot", message: "❌ Command not recognized. Try: help, products, cart, contact", ts: Date.now() / 1000 },
    ]);
    setInput("");
    return;
  }

  // normal fetch if command is allowed
};
